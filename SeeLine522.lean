...
+ int.of_nat (multiset.sum G.loops) - (multiset.card ((multiset.erase G.chains c)))*4 - (multiset.card (G.loops))*8 := sorry

lemma fcv_rem_loop (G : sle) (c ∈ G.loops) : fcv (rem_loop_from_sle G c) =  int.of_nat (multiset.sum G.chains) 
+ int.of_nat (multiset.sum (multiset.erase G.loops c)) - (multiset.card (G.chains))*4 - (multiset.card (multiset.erase G.loops c))*8 := sorry
...
else if G.chains = multiset.cons c (multiset.filter (λ x, x = 3) G.chains) then 6
else 4 :=
begin
split_ifs, unfold tb, sorry, sorry,sorry,sorry,sorry,
end
...
lemma exhaustive_cases (G : sle) : (G.chains = multiset.cons 3 0 ∧ multiset.card G.loops = 1)
...
lemma Lemma_two_a (G : sle) (m n : nat) ( Hm : 3 ≤ m) (Hn : 3 ≤ n ):   m ≤ n → int.nat_abs ( value ( cons_chain_to_sle G m Hm) - value ( cons_chain_to_sle G n Hn )) ≤ n - m :=
begin 
intro x,  induction n with s Ps,  induction m with t Pt, exfalso, have N : ¬ 3 ≤ 0, exact dec_trivial, exact false.elim (N Hn), 
exfalso, have N : ¬ 3 ≤ 0, exact dec_trivial, exact false.elim (N Hn), unfold value,
-- rw le_iff_eq_or_lt at Hn, cases Hn, exfalso, 
--rw succ_eq_add_one at Hn_1, have H1 :  succ 1 + 1 = s + 1, simp, exact Hn_1, have F : succ 1 = s, exact add_right_cancel H1, 
--rw ← F at x, 
       sorry,
end

lemma Lemma_two_b (G : sle) (m n : nat) ( Hm : 4 ≤ m) (Hn : 4 ≤ n ) (Pn : 2 ∣ n) (Pm : 2 ∣ m)  :   m ≤ n → int.nat_abs ( value ( cons_loop_to_sle G m Hm Pm) - value ( cons_loop_to_sle G n Hn Pn )) ≤ n - m :=
begin 
intro x,  induction n with s Ps,  induction m with t Pt, exfalso, have N : ¬ 4 ≤ 0, exact dec_trivial, exact false.elim (N Hn), 
exfalso, have N : ¬ 4 ≤ 0, exact dec_trivial, exact false.elim (N Hn), unfold value,
-- rw le_iff_eq_or_lt at Hn, cases Hn, exfalso, 
--rw succ_eq_add_one at Hn_1, have H1 :  succ 1 + 1 = s + 1, simp, exact Hn_1, have F : succ 1 = s, exact add_right_cancel H1, 
--rw ← F at x, 
       sorry,
...
rw le_iff_eq_or_lt at K, cases K, sorry, exact lt_trans (dec_trivial) K, rw card_pos_iff_exists_mem at P, cases P with a Ha,
have T : tb (rem_chain_from_sle G a) = tb G ∧ cv (rem_chain_from_sle G a) ≥ 2, 
split, rw tb_rem_chain, unfold tb, have Q : ¬ 1 ≥ 2, exact dec_trivial, split_ifs, 
refl, exfalso, rw h.left at y, simp at y,  exact false.elim (Q y),
exfalso, rw h.left at y, simp at y, exact false.elim (Q y),
exfalso, rw h.left at y, simp at y, exact false.elim (Q y), 
exfalso, rw h.left at y, simp at y, exact false.elim (Q y),
exfalso, dunfold size at h_2, rw h_1 at h_2, simp at h_2,  rw h_2 at y, simp at y, have R : ¬ 0 ≥ 2, exact dec_trivial, 
exact false.elim (R y),  
refl,
...
refl, 
exact Ha, 
unfold cv, rw tb_rem_chain, rw fcv_rem_chain, rw H.right, simp, have Q : ¬ 1 ≥ 2, exact dec_trivial, split_ifs, 
exfalso, rw h at y, simp at y, exact false.elim (Q y), sorry,
exact Ha,
exact Ha,
fapply exists.intro, exact a, fapply exists.intro, exact Ha, exact T, 
cases H, dunfold size at y, right, have L : G.loops ≠ 0, finish, 
have LL : ∃ (a : ℕ), a ∈ G.loops, exact exists_mem_of_ne_zero L, cases LL with a Ha, 
have T : tb (rem_loop_from_sle G a) = tb G ∧ cv (rem_loop_from_sle G a) ≥ 4,  
split, rw tb_rem_loop, unfold tb, have Q : ¬ 1 ≥ 2, exact dec_trivial, split_ifs, 
refl,
exfalso, rw [h.left, h.right] at y, simp at y, exact false.elim (Q y),
exfalso, rw [h.left, h.right] at y, simp at y, exact false.elim (Q y),
exfalso, rw [h.left, h.right] at y, simp at y, exact false.elim (Q y),
exfalso, dunfold size at h_2, rw h_1 at h_2, simp at h_2, exact false.elim (L h_2),  
refl,
exfalso, exact false.elim (L (zero_size G h_3).right), 
exfalso, have P : filter (λ (x : ℕ), x ≥ 4) (G.chains) = G.chains, exact (H.left).symm, 
have d : 3 ∈ multiset.cons 3 0, exact mem_cons_self 3 0, rw ← h_4 at d, rw mem_erase_dup at d, 
rw filter_eq_self at P, sorry,
refl,
exfalso, exact false.elim (L (zero_size G h_4).right), 
refl,
exfalso, exact false.elim (L (zero_size G h_4).right), 
refl,
exact Ha,
unfold cv, rw tb_rem_loop, rw fcv_rem_loop, split_ifs,
...
exact Ha,
fapply exists.intro, exact a, fapply exists.intro, exact Ha, exact T,
cases H, dunfold size at y, right, 
have LL : ∃ (a : ℕ), a ∈ G.loops, sorry, cases LL with a Ha, 
have T : tb (rem_loop_from_sle G a) = tb G ∧ cv (rem_loop_from_sle G a) ≥ 4,  
split, rw tb_rem_loop, unfold tb, have Q : ¬ 1 ≥ 2, exact dec_trivial, split_ifs, 
refl,
...
exact (three_point_two G x y).left,   sorry, sorry, sorry, sorry
