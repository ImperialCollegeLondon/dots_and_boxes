...
{
  long_chains := (multiset.cons a (sle.long_chains G)),
  long_chains_are_long := add_long_all_long (sle.long_chains G) a 3 (sle.long_chains_are_long G) Ha,
  long_loops := (sle.long_loops G),
  long_loops_are_long_and_even := G.long_loops_are_long_and_even
}
...
def cons_loop_to_sle (G : sle) (a:nat) (Ha : a ≥ 4 ) (Pa : 2 ∣ a) : sle :=
{
  long_chains := (sle.long_chains G),
  long_chains_are_long := G.long_chains_are_long,
  long_loops := (multiset.cons a (sle.long_loops G)),
  long_loops_are_long_and_even := begin intros, split,  rw[multiset.mem_cons] at H, cases H, rw H, by exact Ha, 
  sorry, 
...
def fcv (G : sle):= multiset.sum G.long_chains + multiset.sum G.long_loops - (distel (G.long_chains))*4 
...
def tb (G : sle)  :=
if size G = 0 then 0
else if  G.long_loops = 0 then 8
else if  G.long_chains = 0 then 4
--else if (multiset.count 3 G.long_chains) = (multiset.card G.long_chains) then 6
else if multiset.erase_dup (G.long_chains) = (multiset.cons 3 0) then 6
else 4
...
(Hno3chains : G.long_chains.count 3 = 0) (Hno4loops : G.long_loops.count 4 = 0) : value G ≥ 2 :=
begin
  unfold value,
  rw value_aux_eq,
...
lemma loop_and_three_chain_case (G : sle) (n : nat) (Hn : n ≥ 4) (He : 2 ∣ n) : G.long_chains = cons 3 0  → (G.long_loops = cons n 0 ) 
→ (cv G = value G) := 
begin
 intros a b,
unfold cv, unfold value, unfold tb, unfold fcv,
split_ifs, 
exfalso, dunfold size at h, rw b at h, simp at h, by exact false.elim ((pos_add_pos 1 (card (G.long_chains)) one_not_zero h )), 
exfalso, rw [← card_eq_zero] at h_1, finish,
exfalso, have p :  cons 3 0 ≠ 0, by exact cons_ne_zero, by exact false.elim ((eq_neq_mult G.long_chains (cons 3 0) 0 a p) h_2),
rw b, unfold distel, rw [h_3, a, card_cons 3 0, card_zero, erase_dup_cons, erase_dup_zero, ndinsert_zero, card_cons n 0, card_zero] , 
