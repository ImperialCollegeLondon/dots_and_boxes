...
lemma rw_fcv (G : sle) : ∃  (n : ℤ) (H : 0 ≤ n ),  fcv G =  n - count 3 G.chains - 4 * count 4 G.loops - 2 * count 6 G.loops :=
sorry
...
lemma three_point_two_f1 (G : sle) : ((size G) > 0) → (cv G < 2) → 3 ∈ G.chains → value G ≤ 3 ∧ value (rem_chain_from_sle G 3) ≤ 4:=
sorry

lemma three_point_two_f2 (G : sle) : ((size G) > 0) → (cv G < 2) → 4 ∈ G.loops → value (rem_loop_from_sle G 4) ≤ 5 :=
begin 
intros x y z, sorry
end

lemma three_point_two_f3 (G : sle) : ((size G) > 0) → (cv G < 2) → ¬ 3 ∈ G.chains → ¬ 4 ∈ G.loops → value (rem_loop_from_sle G 6) ≤ 4  :=
sorry

theorem three_point_two_f (G: sle) : ((size G) > 0) → (cv G < 2) → if mem 3 G.chains then value G ≤ 3 ∧ value (rem_chain_from_sle G 3) ≤ 4 
